      ...
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-2">
          ...
          {!isLoading && !error && records.length === 0 && (
            <p className="py-6 text-center text-sm text-on-surface-variant">아직 진단 기록이 없어요.</p>
          )}
          {records.map((record) => (
            <button
              type="button"
            ...
            >
              ...
              <span className="text-xs text-primary">
                <Icon name="seedling" /> {record.grade} 등급
              </span>
...
export function MeasurementDetailPage() {
  ...
  return (
    <AppLayout active="records" back>
      <PageIntro
        eyebrow="측정 상세"
        title={`${measurement.measuredAt} 기록`}
        description="측정 항목별 결과와 성장 프로필을 확인해보세요."
      />
      <Card className="p-6">
        <div className="flex items-center justify-between">
          <GradeBadge grade={measurement.grade} />
          <span className="text-sm text-on-surface-variant">
            {measurement.type === 'official' ? '정식 측정' : '자가측정'}
          </span>
        </div>
      ...
      <button
        ...
        className="mt-4 w-full rounded-full border border-error/40 px-5 py-3 text-sm text-error disabled:opacity-50"
      >
...
export function GrowthPage() {
  ...
  const growthQuery = useGrowth(childQuery.data?.id)
  const child = childQuery.data
  const growth = growthQuery.data
  if (childQuery.isLoading || growthQuery.isLoading)
    return (
      <AppLayout active="records">
        <LoadingState message="성장 리포트를 불러오고 있어요…" />
      </AppLayout>
    )
  if (childQuery.error || growthQuery.error)
    return (
      <AppLayout active="records">
        <ErrorState
          message="성장 리포트를 불러오지 못했어요."
          onRetry={() => {
  ...
  if (!growth?.series.length)
    return (
      <AppLayout active="records">
        <EmptyState message="성장 추이를 표시할 측정 기록이 없어요." />
      </AppLayout>
    )
  ...
  const metrics = (growth?.series ?? []).slice(0, 4).map((series, index) => {
    ...
    const change =
      latest && previous
        ? `${latest.value - previous.value >= 0 ? '+' : ''}${Math.round(latest.value - previous.value)}%`
        : ''
